...
namespace Mono.Cecil {
	...
	static class ModuleWriter {
		...
		public static void WriteModuleTo (ModuleDefinition module, Stream stream, WriterParameters parameters)
		{
			/*Telerik Authorship*/
			//
			// ILOnly flag indicates that there is only IL code in the module.
			// However, when exposing managed methods as unmanaged, it is a necessity to
			// remove it, since the loader would ignore .reloc sections, which is vital
			// for exposing managed code as unmanaged.
			// We have commented this exception throw, in order not to fail on such
			// assemblies, having in mind that we don't need the written assembly to be
			// exactly the same as the original one.  Moreover, having commented these
			// lines, doesn't affect the .corflags (ModuleAttributes) section -- they are
			// written the value they have been read.
			//
			...
			var symbol_writer = GetSymbolWriter (module, fq_name, symbol_writer_provider, /*Telerik Authorship*/ parameters.MethodsVariableDefinitionToNameMap);
...
//#endif
			var metadata = new MetadataBuilder (module, fq_name,
				symbol_writer_provider, symbol_writer, /*Telerik Authorship*/ parameters.MethodsVariableDefinitionToNameMap);
...
//#endif
			...
			/*Telerik Authorship*/ Dictionary<MethodDefinition, Dictionary<VariableDefinition, string>> methodsVariableDefinitionToNameMap)
		{
			...
			return symbol_writer_provider.GetSymbolWriter (module, fq_name, /*Telerik Authorship*/ methodsVariableDefinitionToNameMap);
		}
	}
	...
	sealed class MetadataBuilder {
		...
		/*Telerik Authorship*/
		readonly Dictionary<MetadataToken, MetadataToken> method_body_original_local_var_token;
