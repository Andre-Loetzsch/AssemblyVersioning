...
namespace Mono.Cecil
{
    ...
    [Serializable]
    public class ResolutionException(MemberReference member) : Exception("Failed to resolve " + member.FullName)
    {
        public MemberReference Member { get; } = member ?? throw new ArgumentNullException(nameof(member));

        public IMetadataScope Scope
        {
            get
            {
                if (this.Member is TypeReference type) return type.Scope;

                var declaringType = this.Member.DeclaringType;
                if (declaringType != null) return declaringType.Scope;
    ...
    public class MetadataResolver(IAssemblyResolver assemblyResolver) : IMetadataResolver
    {
        public IAssemblyResolver AssemblyResolver { get; } = assemblyResolver ?? throw new ArgumentNullException(nameof(assemblyResolver));
        ...
        internal TypeDefinition Resolve(TypeReference type, ICollection<string> visitedDlls)
        {
            ...
            switch (scope.MetadataScopeType)
            {
                case MetadataScopeType.AssemblyNameReference:
                    ...
                    var assembly = this.AssemblyResolver.Resolve((AssemblyNameReference)scope, type.Module.ModuleDirectoryPath, architecture, special);
                    if (assembly == null)
                        return null;
