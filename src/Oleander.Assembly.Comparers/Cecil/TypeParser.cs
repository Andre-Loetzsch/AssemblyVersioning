	...
	class TypeParser {
		...
		string ParsePart ()
		{
			...
			while (this.position < this.length && !IsDelimiter (this.fullname [this.position])) {
				if (this.fullname [this.position] == '\\') this.position++;

				part.Append (this.fullname [this.position++]);
			}
		...
		{
			while (this.position < this.length && Char.IsWhiteSpace (this.fullname [this.position])) this.position++;
		}
		...
		string [] ParseNestedNames ()
		{
			...
			while (this.TryParse ('+'))
				Add (ref nested_names, this.ParsePart ());
		...
		int [] ParseSpecs ()
		{
			...
			while (this.position < this.length) {
				switch (this.fullname [this.position]) {
				case '*':
                    this.position++;
		...
		Type [] ParseGenericArguments (int arity)
		{
			...
			for (int i = 0; i < arity; i++) {
				var fq_argument = this.TryParse ('[');
				Add (ref generic_arguments, this.ParseType (fq_argument));
				if (fq_argument) this.TryParse (']');
		...
		string ParseAssemblyName ()
		{
			...
			while (this.position < this.length) {
				var chr = this.fullname [this.position];
				if (chr == '[' || chr == ']')
					break;
