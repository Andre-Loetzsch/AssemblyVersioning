namespace Mono.Cecil.AssemblyResolver
{
    ...
    public class AssemblyName
    {
        ...
        internal string ParentDirectory(TargetPlatform runtime)
        {
            ...
            {
                return string.Format("v4.0_{0}__{1}", this.Version.ToString(), this.PublicKeyTokenAsString());
            }
            else if (runtime == TargetPlatform.CLR_2_3)
            {
                return string.Format("{0}__{1}", this.Version.ToString(), this.PublicKeyTokenAsString());
            }
            return string.Empty;
        }
        ...
        {
            return this.PublicKeyToken != null
                                                 ? string.Join(string.Empty, this.PublicKeyToken.Select(b => b.ToString("x2")).ToArray())
                                                 : string.Empty;
        }
